...
/// <complexity>Time: O(1) - Only ever does one operation. Space: O(1) - No Variables are used.</complexity>
	private static int FirstElement(int[] arr) => arr[0];
...
/// <complexity>Time: O(n²) - Adds a substantial amount of operations with each new item. Space: O(1) - Only the combo, and increment, variables are used, the addition is discarded.</complexity>
